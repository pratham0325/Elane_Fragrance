...
async function main() {
  ...
  if (!shippingAddress) throw new Error('Seed customer address is required');
  ...
  // ── PRODUCTS
  const placeholder = 'https://images.unsplash.com/photo-1541643600914-78b084683702?w=800&q=80';
  const thumbnails: Record<string, string> = {
    'ÉLANÉ NOIR': 'https://images.unsplash.com/photo-1541643600914-78b084683702?w=600&q=80',
    'ÉLANÉ ÉCLAT': 'https://images.unsplash.com/photo-1592945403407-9caf930c5d3a?w=600&q=80',
    'ÉLANÉ SANTAL': 'https://images.unsplash.com/photo-1566977776052-6e61e35bf9be?w=600&q=80',
    'ÉLANÉ VELVET': 'https://images.unsplash.com/photo-1594736797933-d0401d2e59ff?w=600&q=80',
    'ÉLANÉ OUD IMPÉRIAL': 'https://images.unsplash.com/photo-1589782431553-1d43a9e1b3a8?w=600&q=80'
  };
  ...
  const productDocs = await Product.insertMany(
    PERFUMES.map((p) => ({
      ...
      thumbnail: thumbnails[p.name] ?? placeholder,
      images: [thumbnails[p.name] ?? placeholder],
      brand: 'ÉLANÉ',
  ...
  console.log('   Admin  → elane.com / 1234');
