...
const productSchema = new Schema<IProduct>(
  {
    ...
    // ── Scent Intelligence ────────────────────────────────────────────────
    // aiEmbedding is `select: false` so it never bloats normal product reads.
    aiEmbedding: { type: [Number], select: false, default: undefined },
    aiEmbeddingModel: { type: String },
    aiEmbeddingUpdatedAt: { type: Date },
    semanticText: { type: String, select: false }
  },
  { timestamps: true }
);
...
productSchema.index({ aiEmbeddingUpdatedAt: 1 });
